...
(function() {
  angular.module('omega').controller('IoCtrl', function($scope, $rootScope, $window, $http, omegaTarget, downloadFile) {
    ...
    omegaTarget.state(['web.restoreOnlineUrl', 'webdavUrl', 'webdavUsername', 'webdavPassword', 'lastWebdavSync', 'lastWebdavState']).then(function(_arg) {
      var lastWebdavState, lastWebdavSync, url, webdavPassword, webdavUrl, webdavUsername;
      url = _arg[0], webdavUrl = _arg[1], webdavUsername = _arg[2], webdavPassword = _arg[3], lastWebdavSync = _arg[4], lastWebdavState = _arg[5];
      if (url) {
        $scope.restoreOnlineUrl = url;
      }
      if (webdavUrl) {
        $scope.webdavUrl = webdavUrl;
      }
      if (webdavUsername) {
        $scope.webdavUsername = webdavUsername;
      }
      if (webdavPassword) {
        $scope.webdavPassword = webdavPassword;
      }
      $scope.lastWebdavSync = new Date(lastWebdavSync || Date.now());
      return $scope.lastWebdavState = lastWebdavState || '';
    });
    ...
    $scope.enableOptionsSync = function(args) {
      ...
      enable = function() {
        if (!$scope.webdavUrl || !$scope.webdavUsername || !$scope.webdavPassword) {
          $rootScope.showAlert({
            type: 'error',
        ...
        args.webdavUrl = $scope.webdavUrl;
        args.webdavUsername = $scope.webdavUsername;
        args.webdavPassword = $scope.webdavPassword;
    ...
    return $scope.resetOptionsSync = function() {
      ...
      return omegaTarget.resetOptionsSync({
        webdavUrl: $scope.webdavUrl,
        webdavUsername: $scope.webdavUsername,
        webdavPassword: $scope.webdavPassword
      }).then(function() {
        return $rootScope.applyOptionsConfirm().then(function() {
          return $window.location.reload();
        });
