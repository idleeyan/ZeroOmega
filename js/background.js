  ...
  zeroBackground = function(zeroStorage, opts) {
    ...
    if (sync && options && builtInSyncStorage) {
      builtInSyncStorage.watch([BUILTINSYNCKEY], function(changes, opts) {
        var builtInSyncConfig, lastWebdavETag, webdavPassword, webdavUrl, webdavUsername;
        if (opts == null) {
          opts = {};
        }
        builtInSyncConfig = changes[BUILTINSYNCKEY];
        if (builtInSyncConfig) {
          webdavUrl = builtInSyncConfig.webdavUrl, webdavUsername = builtInSyncConfig.webdavUsername, webdavPassword = builtInSyncConfig.webdavPassword, lastWebdavETag = builtInSyncConfig.lastWebdavETag;
          state.set({
            webdavUrl: webdavUrl,
          ...
          if (sync.enabled) {
            console.log('check webdav change', lastWebdavETag);
            sync.init({
              webdavUrl: webdavUrl,
              webdavUsername: webdavUsername,
              webdavPassword: webdavPassword
            ...
            return state.get({
              ...
              if (syncConfig.lastWebdavETag !== lastWebdavETag) {
                console.log('no match last webdav etag, will check change', syncConfig.lastWebdavETag);
